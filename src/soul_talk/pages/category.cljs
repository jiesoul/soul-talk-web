...
(def columns  [{:title "name" :data-index "name"}
                 {:title "操作" :data-index "tags"}])
...
(defn categories-list []
  (r/with-let [categories (subscribe [:categories])
    ...
    (when @user
      (js/console.log (js/JSON.stringify (clj->js @categories)))
      [:div
       [:> antd/Table {:columns     columns
                       :data-source (js/JSON.stringify (clj->js @categories))
